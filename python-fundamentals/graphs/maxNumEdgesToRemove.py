# O(N) space and time 
# traverse edges once; 

def find(i, parent):
    if i != parent[i]:
        parent[i] = find(parent[i], parent)
    return parent[i]


def union(pi, pj, parent, rank):
    if rank[pi] < rank[pj]:
        parent[pi] = pj
        rank[pj] += rank[pi]
    else: 
        parent[pj] = pi
        rank[pi] += rank[pj]

# i = 0 means nothing; nodes = 1, 2, ... n

def maxNumEdgesToRemove(n, edges):
    parent1, parent2 = [i for i in range(n + 1)], [i for i in range(n + 1)]
    rank1, rank2 = [1 for _ in range(n + 1)], [1 for _ in range(n + 1)]
    edges.sort(key = lambda x: -x[0]) # process the type 3 edges first
    count = 0

    for e_type, u, v in edges: 
        # print(u,v,e_type)
        if e_type == 3: 
            ## union find for 1
            for parent, rank in zip([parent1, parent2], [rank1, rank2]):
                pu = find(u, parent)
                pv = find(v, parent)
                if pu != pv: 
                    union(pu, pv, parent, rank)
                else: 
                    # print(u, v, e_type, 'parents are same', pu, pv)
                    count += .5
            
        if e_type == 2: 
            pu = find(u, parent2)
            pv = find(v, parent2)
            if pu != pv: 
                union(pu, pv, parent2, rank2)
            else: 
                count += 1

        if e_type == 1: 
            pu = find(u, parent1)
            pv = find(v, parent1)
            if pu != pv: 
                union(pu, pv, parent1, rank1)
            else: 
                count += 1
        
    for i in range(1, len(parent1)):
        parent1[i] = find(i, parent1)
        parent2[i] = find(i, parent2) 
    
    # print(parent1, parent2)
    pset1, pset2 = len(set(parent1[1:])), len(set(parent2[1:]))
    # print(pset1, pset2)
    if pset1 > 1 or pset2 > 1: 
        return -1
    elif pset1 == pset2 == 1:
        return int(count)
    else: 
        print('something bad here')

# n = 4
# edges = [[3,2,3],[1,1,2],[2,3,4]]

# n = 4
# edges = [[3,1,2],[3,2,3],[1,1,4],[2,1,4]]

# n = 4
# edges = [[3,1,2],[3,2,3],[1,1,3],[1,2,4],[1,1,2],[2,3,4]]

n = 13
edges = [[1,1,2],[2,1,3],[3,2,4],[3,2,5],[1,2,6],[3,6,7],[3,7,8],[3,6,9],[3,4,10],[2,3,11],[1,5,12],[3,3,13],[2,1,10],[2,6,11],[3,5,13],[1,9,12],[1,6,8],[3,6,13],[2,1,4],[1,1,13],[2,9,10],[2,1,6],[2,10,13],[2,2,9],[3,4,12],[2,4,7],[1,1,10],[1,3,7],[1,7,11],[3,3,12],[2,4,8],[3,8,9],[1,9,13],[2,4,10],[1,6,9],[3,10,13],[1,7,10],[1,1,11],[2,4,9],[3,5,11],[3,2,6],[2,1,5],[2,5,11],[2,1,7],[2,3,8],[2,8,9],[3,4,13],[3,3,8],[3,3,11],[2,9,11],[3,1,8],[2,1,8],[3,8,13],[2,10,11],[3,1,5],[1,10,11],[1,7,12],[2,3,5],[3,1,13],[2,4,11],[2,3,9],[2,6,9],[2,1,13],[3,1,12],[2,7,8],[2,5,6],[3,1,9],[1,5,10],[3,2,13],[2,3,6],[2,2,10],[3,4,11],[1,4,13],[3,5,10],[1,4,10],[1,1,8],[3,3,4],[2,4,6],[2,7,11],[2,7,10],[2,3,12],[3,7,11],[3,9,10],[2,11,13],[1,1,12],[2,10,12],[1,7,13],[1,4,11],[2,4,5],[1,3,10],[2,12,13],[3,3,10],[1,6,12],[3,6,10],[1,3,4],[2,7,9],[1,3,11],[2,2,8],[1,2,8],[1,11,13],[1,2,13],[2,2,6],[1,4,6],[1,6,11],[3,1,2],[1,1,3],[2,11,12],[3,2,11],[1,9,10],[2,6,12],[3,1,7],[1,4,9],[1,10,12],[2,6,13],[2,2,12],[2,1,11],[2,5,9],[1,3,8],[1,7,8],[1,2,12],[1,5,11],[2,7,12],[3,1,11],[3,9,12],[3,2,9],[3,10,11]]
# 114

print(maxNumEdgesToRemove(n, edges))