...
def maximumSetSize(nums1, nums2):
    ...
    # print(both)
    aOnly = min(len(nums1) - len(both), n//2)
    bOnly = min(len(nums2) - len(both), n//2)
    return min( aOnly + bOnly + len(both), n)
...
# n = len(nums1)
# nums1, nums2 = set(nums1), set(nums2)
# print("len of nums1: {}, len of nums2: {}, n: {}".format(len(nums1), len(nums2), n))
# both = nums1 & nums2
# v1 = min(len(nums1) - len(both), n)
# v2 = min(len(nums2) - len(both), n)
